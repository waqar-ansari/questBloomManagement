(function () {
  ...
  // const isHome = window.location.pathname === "/";
  // const isFirstRun = !sessionStorage.getItem("hasVisited");

  // if (isHome && isFirstRun) {
  //   console.log("smoother paused true");
  //   smoother.paused(true);
  // } else {
  //   console.log("smoother paused false");
 ...
 cards.forEach((card, index) => {
  ...
  const updateProgress = (instance) => {
    if (percentage) {
      percentage.textContent = `${Math.round(
        (instance.progressedCount * 100) / wrapImg.length
      )}% Loading...`;
    }
  };
  ...
  const Preload = () => {
    if (
      ...
      return; // Exit if preloader elements are missing
    }
    ...
    tl.to(percentage, {
      ...
      .from(
        loaderText,
        {
          autoAlpha: 0,
          color:
            "random(['#ccd0db', '#615651', '#BF9865', '#455d7a', '#fbf2d5'])",
          duration: 2.8,
          scale: 0,
          y: 80,
          rotationX: 270,
  ...
  $(document).ready(function () {
    ...
    // Stop Scroll
    document
      .querySelector(".js-nav-open-close")
  ...
  document
    .querySelectorAll(".js-horiz-loop .wrap-horiz-loop")
    .forEach((ticker) => {
      let totalDistance;
  ...
  function splitTextFadeUpExport() {
    const splitTextFadeUpTargets = gsap.utils.toArray(
    ...
    splitTextFadeUpTargets.forEach((elem) => {
      ...
      ScrollTrigger.create({
        ...
        onEnter: () =>
          splitTextTlFunction(elem, splitTextLines, splitFadeUpElements),
      });
    });
  }
  ...
  const SplitCharsLetter = document.querySelectorAll(
  ...
  ScrollTrigger.batch(revealLeft, {
    start: "top 80%",
    onEnter: (elem) =>
      elem.forEach((j, k) => j.animation.delay(k * 0.9).restart(true)),
    once: true,
  });
  ...
  ScrollTrigger.batch(revealRight, {
    start: "top 80%",
    onEnter: (elem) =>
      elem.forEach((x, z) => x.animation.delay(z * 0.9).restart(true)),
    once: true,
  });
  ...
  Fade.forEach((item) => {
    ...
    tl.set(item, { autoAlpha: 1 }).from(item, {
      ease: "expo.inOut",
      duration: 1.5,
      autoAlpha: 0,
    });
  ...
  ScrollTrigger.batch(Fade, {
    start: "top 90%",
    onEnter: (elem) =>
      elem.forEach((x, z) => x.animation.delay(z * 0.1).restart(true)),
    once: true,
  });
  ...
  scaleImg.forEach((item) => {
    ...
    tl.set(scaleImg, { xPercent: -50, yPercent: -50 }).to(item, {
      scale: 0.6667,
      ease: "back",
    });
  });
  ...
  scaleImg2.forEach((item) => {
    ...
    tl.set(scaleImg2, { autoAlpha: 0, xPercent: -50, yPercent: -50 }).to(item, {
      scale: 0.6667,
      ease: "back",
      autoAlpha: 1,
    });
  ...
  mm.add("(min-width: 768px)", () => {
    ...
    fadeImage.forEach((image) => {
      ...
      tl.set(image, { autoAlpha: 0, scale: 0.8 })
        .fromTo(
          image,
          {
            autoAlpha: 0,
...
function throttle(delay, noTrailing, callback) {
 ...
 if (!wrapCont) return console.log("No drag slider found");
    ...
    const setBounds = () => {
      ...
      const newX =
        (sliderWidth / 100) *
        (gsap.getProperty(DragProxy, "x") / (prevSliderWidth / 100));
    ...
    console.log("Slider initialized", draggable);
  });
  /*--------------- PARALLAX FOOTER ---------------*/
